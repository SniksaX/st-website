...
type DonationMonth = { month: string; total: number; transactions: number; recurringBase: number; oneTimeTotal: number }
...
const DONATION_MONTHS: DonationMonth[] = [
  { month: 'Oct. 25', total: 440, transactions: 51, recurringBase: 107, oneTimeTotal: 333 },
  { month: 'Nov. 25', total: 239, transactions: 30, recurringBase: 246, oneTimeTotal: 100 },
  { month: 'Dec. 25', total: 26, transactions: 6, recurringBase: 262, oneTimeTotal: 10 },
  { month: 'Jan. 26', total: 87, transactions: 6, recurringBase: 279, oneTimeTotal: 70 },
  { month: 'Fev. 26', total: 71, transactions: 12, recurringBase: 320, oneTimeTotal: 30 },
  { month: 'Mar. 26', total: 74, transactions: 17, recurringBase: 379, oneTimeTotal: 15 },
  { month: 'Avr. 26', total: 728, transactions: 84, recurringBase: 598, oneTimeTotal: 509 },
  { month: 'Mai 26', total: 58, transactions: 11, recurringBase: 592, oneTimeTotal: 35 },
]
      ...
      <div className="cbt-donation-head">
        ...
        <div className="cbt-donation-summary">
          ...
          <div className="cbt-donation-metrics">
            ...
            <div>
              <strong>{firstAboveThreshold?.month ?? 'Fev. 26'}</strong>
        ...
        {donationMonths.map((item) => (
          <div key={item.month} className={`cbt-donation-month${item.recurringBase >= threshold ? ' cbt-donation-month-above' : ''}`}>
            ...
            <div className="cbt-donation-meta">
              ...
              <span>{formatStatValue(item.oneTimeTotal, 'EUR')} ponctuels</span>
            </div>
          </div>
        ))}
      </div>
...
function EngagementBenchmarkGraph() {
  const rows = [
    ...
    { label: 'Sans Transition', sub: 'Engagement moyen consolidé', value: '21,7 %', pct: 86.8, muted: false },
  ]

  return (
    <div className="cbt-er-graph">
      <div className="cbt-er-ghost" aria-hidden>21,7</div>
      ...
      <div className="cbt-er-graph-main">
        <span className="cbt-er-graph-value grad-text" data-count={21.7} data-suffix="%">0</span>
        <div className="cbt-er-graph-copy">
          <span>% engagement moyen</span>
          <strong>x3,6 le benchmark secteur</strong>
        </div>
      </div>
