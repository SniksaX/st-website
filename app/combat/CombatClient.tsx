...
const HERO_STATS: StatItem[] = [
  ...
  { label: 'Donateurs·trices', sub: 'Récurrents sans défiscalisation', count: 155, icon: <Heart size={20} /> },
]
...
const ASK_ITEMS: AskItem[] = [
  { idx: '01', title: 'Premiers postes salariés', body: "Salarier le président (direction éditoriale et pilotage de production), la responsable éditoriale (coordination Fokus et formats studio) et le responsable communication. Ces trois rôles constituent le noyau irremplaçable de la production. Le sujet n'est plus la preuve de concept mais la capacité à tenir." },
...
const ROADMAP_ITEMS: AskItem[] = [
  ...
  { idx: '04', title: 'Base de dons structurée', body: "Objectif : passer de 155 à 220+ donateurs·trices d'ici fin 2026 puis 350+ à horizon fin 2027." },
]

const COMPARE_ROWS: CompareRow[] = [
  { label: 'Dons mensuels actuellement encaissés', value: '592 EUR', pct: 5, gradient: true },
  { label: 'Coût mensuel brut des trois postes salariés', value: '~ 10 925 EUR', pct: 100, dark: true },
  { label: "Coût d'un Fokus en tarif prestataire", value: '400 à 800 EUR', pct: 7 },
]
...
const FINANCE_POINTS: FinancePoint[] = [
  ...
  },
  {
    ...
    title: "Les trois postes salariés représentent dix-huit fois les revenus mensuels actuels.",
    body: "Le mensuel actif reste insuffisant face à un minimum d’environ 10 925 EUR brut mensuel pour les trois postes, avant même les déplacements, logiciels, stockage ou renouvellements de matériel.",
  },
  {
    kicker: 'Enjeu réel',
...
const BUDGET_SUMMARY: BudgetSummaryItem[] = [
  { label: 'Masse salariale', amount: '131 100 EUR', note: 'Trois postes salariés à temps plein, charges incluses.', primary: true },
    ...
    <div className="cbt-chart-wrap cbt-rv cbt-d1">
      <svg className="cbt-chart-svg" viewBox="0 0 900 320" id="cbt-projection-svg">
        <defs>
          ...
          <linearGradient id="cbt-proj-legend-lg" gradientUnits="userSpaceOnUse" x1="580" y1="18" x2="608" y2="18">
            <stop offset="0%" stopColor="oklch(0.72 0.27 290)" />
            <stop offset="48%" stopColor="oklch(0.78 0.22 330)" />
            <stop offset="100%" stopColor="oklch(0.85 0.25 40)" />
          </linearGradient>
        ...
        {/* Legend */}
        <path d="M580,18 L608,18" stroke="url(#cbt-proj-legend-lg)" strokeWidth="2.5" />
        <text x="613" y="22" className="cbt-axis-label">Avec financement</text>
        <g className="cbt-proj-info-group">
          <circle cx="736" cy="18" r="7" fill="oklch(0.72 0.27 290 / 0.12)" stroke="oklch(0.72 0.27 290 / 0.45)" strokeWidth="1" style={{ cursor: 'default' }} />
          <text x="736" y="22" textAnchor="middle" className="cbt-proj-info-i">i</text>
          <g className="cbt-proj-tooltip">
            <rect x="488" y="32" width="248" height="68" rx="5" fill="oklch(0.12 0.02 270)" stroke="oklch(0.72 0.27 290 / 0.3)" strokeWidth="1" />
            <text x="500" y="48" className="cbt-proj-tooltip-text">Investissement à temps plein ·</text>
            <text x="500" y="63" className="cbt-proj-tooltip-text">dons réinvestis en production ·</text>
            <text x="500" y="78" className="cbt-proj-tooltip-text">hausse de cadence (Tribunal des Médias)</text>
          </g>
        </g>
...
export default function CombatClient() {
  ...
  const liveDonors = Math.max(0, donProgress?.monthlyActive ?? 155)
  ...
  const compareRows = COMPARE_ROWS.map((row) => {
    ...
    return {
      ...
      pct: Math.min(100, Math.round((monthlyAmount / 10925) * 100)),
    }
  })
